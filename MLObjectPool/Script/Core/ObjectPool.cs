...
namespace MLObjectPool
{
    public sealed class ObjectPool : Singleton<ObjectPool>
    {
        ...
        public PrefabPool CreatePrefabPool(string name, GameObject go, int size, bool autoExpand)
        {
        ...
        /// <param name="autoExpand">是否自动扩容</param>
        public Pool<T> CreatePool<T>(string name, int size, bool autoExpand) where T : new()
        {
            if (poolMap.ContainsKey(name))
            {
                Log.PrintError($"{name} has already exist. please type other name for ObjectPool");
